...
// Caminho absoluto para o mockup index.html da sandbox v2
const mockupPath = `file://${path.resolve(__dirname, '../mockup/index.html').replace(/\\/g, '/')}`;
...
test.describe('Gauntlet Loop v2 — Suíte Completa de Testes (T01 a T24)', () => {
  ...
  test('T01 — Básico Fresar: topo reto, Aço 1045, MD, desbaste, Ø10mm, Z=4', async ({ page }) => {
    await page.selectOption('[data-testid="select-familia"]', 'fresar');
    ...
    await page.selectOption('[data-testid="select-material-ferramenta"]', 'MD');
    await page.selectOption('[data-testid="select-operacao"]', 'desbaste');
  ...
  test('T02 — Troca de Família sem Reconstruir Tela', async ({ page }) => {
    ...
    await page.selectOption('[data-testid="select-familia"]', 'furar');
  ...
  test('T04 — Zonas Fixas (1 a 6) visíveis nas 4 Famílias', async ({ page }) => {
    ...
    for (const fam of familias) {
      await page.selectOption('[data-testid="select-familia"]', fam);
  ...
  test('T11 — Torque Excedido exibe Alerta Vermelho / Bloqueio', async ({ page }) => {
    await page.selectOption('[data-testid="select-familia"]', 'roscar');
  ...
  test('T12 — Vc com HSS é aproximadamente 29% do Vc com Metal Duro', async ({ page }) => {
    ...
    await page.selectOption('[data-testid="select-material-ferramenta"]', 'HSS');
  ...
  test('T22 — Mandrilar ap por lado e alerta de L/D > 5', async ({ page }) => {
    await page.selectOption('[data-testid="select-familia"]', 'mandrilar');
