  ...
  function calcularResultados(fam) {
    ...
    if (fam === 'fresar') {
        ...
        oQueMexer: [
          {
            ...
            acao: `Reduza o balanço (L) para ${formatDec(D * 4.0, 0)} mm (hoje ${inp.l} mm).`,
            explicacao: 'A relação L/D retorna ao patamar rígido de 4,0, eliminando deflexão e marcas de trepidação na peça.',
            contrapartida: 'Em troca: o alcance axial da ferramenta diminui.'
          },
          {
      ...
      return {
        nivel: 'NORMAL',
        alertaTitulo: 'Nenhuma condição fora da faixa nesta combinação',
        alertaCorpo: `Balanço relativo L/D em ${formatDec(ld, 1)} dentro da zona de rigidez ideal (< 4,0).`,
        alertaExtra: null,
        z3Params: [
    ...
    const z7Cards = resData.resumoGeral.map(c => `
      <div class="rcard">
        ...
        <div style="display:flex;align-items:baseline;gap:6px">
          <span class="u20" style="${c.isAlert ? 'color:var(--st-warn-ink)' : ''}">${c.val}</span>
            ...
            <div style="display:flex;flex-direction:column;gap:8px">
              <div class="field">
                <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:4px">
                  ...
                  <button type="button" class="btn-link-cfg" id="link-cfg-ferramentas" style="background:none;border:none;color:var(--tx-brand);cursor:pointer;font-size:11px;text-decoration:underline">Gerenciar na oficina</button>
                </div>
                <div class="selbox">
  ...
  function renderSettingsView() {
    ...
    const tiposFerramentasHtml = FENIX_MOCK_DATA.tiposFerramenta.map(tipo => {
      ...
      const itensHtml = ferramentasDoTipo.length === 0
        ? `<div class="cap" style="padding:10px 14px;color:var(--tx-3)">Nenhuma ferramenta cadastrada nesta categoria. Adicione uma acima.</div>`
      ...
      return `
        <div class="tipo-ferramenta-grupo" style="margin-bottom:14px">
